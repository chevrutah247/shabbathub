...
export default function CatalogPage() {
  ...
  useEffect(() => {
    fetch(SUPABASE_URL + '/rest/v1/issues?is_active=eq.true&order=gregorian_date.desc&limit=100&select=id,title,pdf_url,page_count,gregorian_date,publication_id,thumbnail_url', {
  ...
  return (
    <div className="min-h-screen bg-cream">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <p className="text-gray-600 mb-6">Газеты, недельные главы Торы и печатные материалы к Шаббату</p>
            ...
            <article key={doc.id} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              <div className="relative">
                ...
                <div className="absolute left-2 top-2 flex flex-col gap-1">
                  <button className="w-6 h-6 bg-blue-600 text-white rounded-full flex items-center justify-center hover:bg-blue-700">
                    <FacebookIcon />
                  </button>
                  <button className="w-6 h-6 bg-green-500 text-white rounded-full flex items-center justify-center hover:bg-green-600">
                    <WhatsAppIcon />
                  </button>
                  <button className="w-6 h-6 bg-sky-500 text-white rounded-full flex items-center justify-center hover:bg-sky-600">
                    <TelegramIcon />
                  </button>
                </div>

                <div className="absolute right-2 top-2 flex flex-col gap-1">
                  <button className="w-6 h-6 bg-white/90 text-gray-400 rounded-full flex items-center justify-center hover:text-red-500 shadow-sm">
                    <Heart size={12} />
                  </button>
                  <button className="w-6 h-6 bg-white/90 text-gray-400 rounded-full flex items-center justify-center hover:text-primary-600 shadow-sm">
                    <User size={12} />
                  </button>
                </div>
              </div>
